...
test('saved model aliases synchronize existing homes without discovery or a thread database', async t => {
  ...
  assert.match(config,/model_provider = "cma_router"/);
  assert.match(config,/model = "ark-code-latest"/);
  const catalog=JSON.parse(await fs.readFile(path.join(prepared.homePath,'model-catalog.json'),'utf8'));
  assert.ok(catalog.models.some(m=>m.slug==='ark-code-latest'));
  assert.equal((await store.route(f.route.id)).model,'new-alias');
  assert.equal(f.calls(),0);
  ...
  assert.equal(await fs.readFile(path.join(prepared.homePath,'config.toml'),'utf8'),config);
});
