...
在 Model Router 创建的 API 工作窗口里，选择「自动选择模型」后，网关会对该对话的每一轮请求重新判定任务类别和难度，从连接检查、真实能力验证、额度失败记录与上下文限制生成合格候选快照。只有一个合格远程模型时直接选它；有多个合格远程模型时才交给本机 Laya / Jev 引擎决策。手动选择具体模型时不会被自动路由覆盖。有合格 API 模型时，本地模型只作备用。自动路线失败时只转向同一轮快照里同样合格的备用模型，不会临时抓一个未经验证的模型。为防止流式响应中途失败后把两家模型的文字拼在一起，自动模式先缓存该轮供应商流式正文，确认完整成功后才显示；期间可有等待提示，但正文首字会比手选模型晚出现。

已完成的能力测试在模型 ID、接口、凭据均未变化时供自动模式参考，最长 30 天；连接检查超过 24 小时且备用不足时，网关会先用小请求自动复检，复检不过则不授予资格。供应商明确返回 `No endpoints found` 或当前分组不支持该模型时，该路线的连接资格立即失效，直到后来重新真实验证成功。没有同任务合格备用时仍会明确失败，不保证任何供应商故障都能无条件切换成功。
...
这只作用于 Model Router 管理的第三方 API Codex 工作窗口，不修改官方 ChatGPT Work 的模型菜单，也不会改动已在运行的旧对话。能力验证是短任务测试，不证明模型广告里的 1M 上下文；超过保守窗口的请求会拒绝自动分配。没有合格规划模型时明确报错，不能凭公开排名或模型名称替它通过本机验收。额度不能从一个 Coding 套餐 Key 普遍读出，只能利用供应商真实报错与后续成功记录判断；额度不足持续排除该路线，临时 5xx/限流冷却两分钟。若供应商已输出正文或已经执行工具，不会把另一模型的输出拼到同一轮里。
