# Model Router 3.5.0 使用手册

## 智能执行任务

打开 **Model Router.app**，在首页顶部点击「智能执行任务」。填写任务内容，选择难度和能力类别，再填写答案必须包含的短语，点击「执行并验收」。页面会显示选中的路线、实际调用路线、决策来源、每次网关 request ID、响应模型标识、耗时、验收结果和答案；没有近期验证合格的路线会拒绝执行。详细资格与证据边界见 [智能执行任务](TASK-EXECUTION.md)。

这个入口是 Model Router 自己执行一项任务。**ChatGPT Work 的模型下拉菜单仍是手动选模**，不会因安装 Model Router 自动出现「智能执行任务」。现有 Codex 对话也不会被暗中改模。
